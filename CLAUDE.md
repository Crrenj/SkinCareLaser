...
- **Favicon par thème (client)** : `<link rel=icon>` SSR initial par thème + **`src/components/ThemeFavicon.tsx`** (client) qui lit le thème live via **`GET /api/theme`** (SWR) et réécrit les liens à chaque navigation + retire tout favicon concurrent (sinon figé en SPA / périmé selon la page SSG). `src/app/favicon.ico` **supprimé** (Next l'injectait → logo FARMAU concurrent gagnait sur l'admin).
...
- **`src/components/ThemeFavicon.tsx`** + **`IframeHeightReporter.tsx`** — favicon client par thème (cf. Système de thèmes) ; reporter de hauteur (postMessage) pour l'iframe d'aperçu home admin. (L'ancien `Logo.tsx` est supprimé — utiliser `FarmauLogo`.)
