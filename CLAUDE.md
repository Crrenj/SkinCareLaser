...
- **Phase 8 (branche `chore/next-16`, NON mergée)** : Next **16.2.9** épinglé (Turbopack build vert 36 s, 62 routes prerendues, lang OK, smoke OK — re-vérifié indépendamment) ; `next lint` → **ESLint CLI** (flat config native eslint-config-next) ; `revalidateTag(tag, { expire: 0 })` (nouvelle signature) ; `@types/node` ^22 ; middleware.ts conservé. ⚠️ basée sur le commit Phase 7 → rebase/merge requis avec Phase 9 + L-2 avant adoption.
