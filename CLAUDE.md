...
**Hiérarchie de rôles** (migration `20260529120000`) : `admin_users.role` ∈ `{admin, super_admin}`. `is_user_admin` reste binaire (présence dans la table = accès panel) — **inchangée**. La gestion de l'équipe admin (promouvoir/révoquer/changer de rôle) est réservée aux **super_admin** via `requireSuperAdmin()` (`src/lib/requireAdmin.ts`, + `getAdminRole(userId)`). Garde-fous serveur : pas d'auto-modification, pas de modification d'un AUTRE super_admin (anti-coup / anti-orphelinage → retrait d'un super_admin = en base). UI : page `/admin/admins` (section *Acceso* sidebar) + `GET /api/admin/admins` ; la mutation de RÔLE passe par `PATCH /api/admin/users/[id]` (super-admin only, body `{ isAdmin?, role? }`) ; le PSEUDO d'un admin s'édite via `PATCH /api/admin/admins` (tout admin, body `{ userId, displayName }`, audité). L'owner (`juanlantiguajorge@gmail.com`) est l'unique super_admin (le compte seed `j@gmail.com` a été supprimé le 2026-06-11). **Règle d'affichage des noms** (`src/lib/userName.ts`) : admin → pseudo `display_name` (éditable par l'équipe) ; client → nom + prénom (jamais le pseudo). Appliquée dans `/admin/users`, `/admin/admins`, la recherche client (`/api/admin/users/search`) et la carte identité du sidebar admin.
...
- `src/app/admin/*` — `product`, `marques`, `stock`, `tags`, `messages`, `annonce`, `reservations`, **`users`**, **`newsletter`**, **`blog`**, **`apariencia`** (thème du site), `setup` (diag), `settings`, **`tuto`** (guide d'utilisation intégré, sidebar *Général* : 21 sections — un wireframe annoté + actions/conséquences/gravité + flux par écran ; contenu par locale dans `tuto/_content/sections/{fr,es,en}/*.ts` typé `TutoContent` (modèle pages légales : parité par le COMPILATEUR, pas par les messages JSON — seul le chrome ~16 clés est dans `Admin.tuto`) ; rédigé/vérifié/traduit par agents contre le code réel 2026-06-11)

**API** :
- `src/app/api/admin/*` — **40 fichiers `route.ts`** service-role (toutes `requireAdmin()`, sauf `users/[id]` en `requireSuperAdmin()` ; mutations validées Zod via `src/lib/schemas.ts`) : `products` (+`[id]`, +`with-tags`), `brands` (+`[id]`), `ranges` (+`[id]`), `tags` (+`[id]`), `tag-types` (+`[id]`), `promotions` (+`[id]`), `banners` (+`stats`), `messages`, `reservations`, `reviews` (+`[id]`), `stock` (+`entry`, +`loss`), `expenses` (+`[id]`), `accounting/export`, `upload`, `sidebar-stats`, `users` (GET/PATCH, +`[id]`, +`search`, +`quick-create`), `newsletter` (GET/DELETE/CSV export, +`[id]`), `posts` (GET/POST/PATCH/DELETE), `appearance` (GET/PATCH thème + `revalidateTag`), `settings`, `admins` (super-admin only), `logs` (journal d'audit), `home-layout`, `set-locale`
...
- `stock_entries` (réceptions append-only, 606-ready) + `products.cost_price` = **CMP** (coût moyen pondéré) — RPC `record_stock_entries`/`recompute_cost_price` ; `reservation_items.unit_cost` = snapshot CMP write-once à la vente (`apply_reservation_collection`). `cost_price`/`unit_cost` **masqués à anon/authenticated** (liste blanche de colonnes). Migrations `20260607130000` / `20260608120000`
- `expenses` (charges P&L, admin-only, RLS + service-role, CHECK **9 catégories** dont `merma`, `created_by`) — alimente `/admin/contabilidad`
- `reviews` (avis produits, statut pending/approved/rejected, RLS public SELECT approved + admin modère)
- `stock_losses` (merma / pertes : `quantity`, `unit_cost` snapshot, `reason` vencido/danado/robo/ajuste, `expense_id`, `client_token`) — RPC `record_stock_loss` (décrément + charge `merma` au coût, atomique, idempotent ; migration `20260609130000`)
- `audit_log` (journal d'audit admin : `actor_id`, `action`, `entity`, `summary` ES, `diff` jsonb, `is_high_impact` ; RLS lecture **tous admins**, écriture service-role) — helper `recordAuditLog` via `after()` (migration `20260609120000`)
- `promotions` + `promotion_targets` (campagnes promo % / fixe, cibles produit/brand/range/tag) → fonction `effective_price()` + vue publique `v_product_pricing` (`security_invoker`, aucun coût exposé) ; alimente le snapshot des réservations + l'affichage barré ; RPC `set_promotion_targets` (swap atomique ; migration `20260609140000`)
...
## État du projet (2026-06-11)

### Fait ✅ (sessions 2026-06-10 → 06-11 — exécution du plan de remédiation post-audit, Phases 0→9 + L-2)

Exécution complète de `docs/audits/full-audit-2026-06-10/00-REMEDIATION-PLAN.md` (v2 raffiné). **~11 commits sur `main`** (`4ebfe99` → L-2) + **branche `chore/next-16`** (`77c98fa`, non mergée — décision propriétaire) + **6 migrations appliquées prod** (`20260610190000` → `20260611230000`). Gates verts à chaque palier ; état final : tsc 0 · lint 0 · build vert · **vitest 111 passed | 11 skipped** (122) · parité i18n **2079×3**. Mémoire détaillée : `remediation-2026-06-10-progress`.

- **Phase 0 + G-4a** : `safeJsonLd` (échappement `</script>`) sur les 4 sites JSON-LD ; allowlist images (Supabase Storage + Unsplash seulement, `minimumCacheTTL` 30 j) ; cookie `cart_id` Secure ; 5 fuites `error.message` assainies. **Sentry dormant** (`@sentry/nextjs` ^10 SANS `withSentryConfig` ; `instrumentation.ts` + `instrumentation-client.ts` racine + boundaries + `apiError` ; CSP 3 hôtes ingest ; activer = poser `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN`). **Heartbeat cron** : table `cron_heartbeats` (service-role only) + **`GET /api/health`** (503 si cron muet > 35 min — à brancher sur un uptime monitor) ; cadence `expire-stale-reservations` `*/5`→`*/15` (D-7).
- **Phase 1 — SSG/ISR réellement débloqué (2 → 61 routes prerendues)** : cause racine = `getLocale()` dans le root layout (header next-intl → TOUT l'arbre dynamique depuis `bb95ef6`, les ● du tableau de build étaient trompeurs — la **vérité = `.next/prerender-manifest.json` + headers runtime**). Fix (décision propriétaire) : **plus de `app/layout.tsx`** — 3 ROOT layouts par branche (`[locale]` statique lang exact / `admin` dynamique cookie / `auth` callback) + coquille partagée `src/components/AppHtmlShell.tsx` ; `app/not-found.tsx` racine supprimé. Client public cookieless **`src/lib/supabasePublic.ts`** sur les 9 surfaces publiques (les pages user gardent le client cookies). `generateStaticParams` vide sur les 4 pages `[slug]` (ISR à la demande). ⚠️ `unstable_rootParams()` rend `{}` au prerender en 15.5 (piste éliminée).
- **G-3a + G-3b — tests** : `pricing.test.ts` (15) · `catalogueFilters.test.ts` (47, **contrat des filtres URL**) · `csv.test.ts` (22) · intégration DB **opt-in `RUN_DB_TESTS=1`** (écrit/nettoie la prod, préfixes `g3atest-`/`g3btest-`) : `pricingDb` (8 — %/fixe/MIN/fenêtre/plancher/fan-out/vue anon RLS + **invariant prix affiché ≠ prix facturé** : `unit_price` surchargé survit à `apply_reservation_collection`) et `stockCostDb` (3 — CMP pondéré + idempotence, merma à coût snapshotté, COGS excluant les coûts NULL).
- **Phase 5 + 2-B (migration `20260610211000`)** : D-1 index partiel `is_active` · D-2/D-8 drop des doublons + 5 unused SÛRS (les 8 sur tables vides conservés) · D-3 4 index FK · D-4 **`pg_trgm` + `unaccent` + colonne générée `products.name_search`** (lower+unaccent, GRANT colonne — liste blanche) → `/api/search` et catalogue **accent-insensibles** · D-5 `(SELECT auth.jwt())` · D-6 les 13 policies `FOR ALL` éclatées en INSERT/UPDATE/DELETE (`multiple_permissive` 90→30, résidu documenté) · D-9 autovacuum 5 % · D-10 `tags_with_types` → invoker, **`v_bestsellers` RESTE DEFINER** (COMMENT d'acceptation). **Phase 2-B** : early-return dans `effective_price` (0 promo active → prix de base sans les joins) — filet 8/8 re-passé après.
- **Phase 3 — catalogue en SQL (migration `20260611150000`)** : RPC **`get_catalogue_page`** (SECURITY INVOKER, GRANT anon) = filtrage (marque OR / gamme OR / tags **OR-intra AND-inter** via double NOT EXISTS) + `?q` accent-insensible + tri (dont prix **EFFECTIF**) + pagination clampée + total exact + **facettes « chaque famille exclut son propre filtre »** (vocabulaires complets, zéros inclus) — UNE requête CTE (⚠️ TEMP TABLE interdite en fonction STABLE). `catalogue/page.tsx` : fini le `.limit(500)` + filtres JS ; **`parseFilters` conservé** (compat URL) sur un vocabulaire léger. **A-5** : PDP 6 awaits → 3 étapes (`Promise.all`), similaires ciblés + `.eq('is_active', true)`. **P3-TAGS** : règle colonne-vs-tag documentée (§ Conventions) — les 5 colonnes d'attributs sont **vides 0/353**, les tags portent toute la donnée.
- **Phase 4** : namespace `Admin` (~33 ko) filtré du provider i18n public (−35 % de payload messages).
- **Phase 6 (migration `20260611190000`)** : RPC **`get_dashboard_stats`** + **`get_inventory_valuation`** (service-role ONLY — agrègent `cost_price` ; anon = permission denied vérifié) → dashboard **28 → 5 requêtes**, scan inventaire compta borné. `src/app/global-error.tsx` (styles INLINE — le pipeline CSS peut être la panne) ; `ProductDetailCard` + `CartIcon` supprimés (184 LOC mortes).
- **Phase 7 (migration `20260611210000`)** : **email de confirmation de réservation** `src/lib/reservationEmail.ts` (Resend no-op sans clé, fr/es/en, référence + produits + retrait + TTL + **lien wa.me pré-rempli**) câblé `after()` dans `/api/cart/reserve` (compte + invité-avec-email) — **prérequis DNS/SPF/DKIM Resend côté propriétaire** ; 2 bugs newsletter fixés (re-subscribe non confirmé → renvoi ; confirm → locale de l'abonné). **Effacement de compte réel** `POST /api/account/delete` (confirmation `ELIMINAR`, 403 admin) — ⚠️ **`reservations.user_id` est ON DELETE CASCADE** : l'endpoint détache (`user_id` NULL) + anonymise les `contact_*` AVANT `deleteUser` (sinon perte compta). **Rétention** : `purge_expired_data()` + pg_cron quotidien 03:30 + heartbeat. **Rate-limit** : option `failClosed` (opt-in, défaut fail-open délibéré conservé) + trace `[rate-limit] fail-open/closed` ; fail-closed sur contact/newsletter/guest-reserve ; couverture étendue `/api/cart` écritures (60/min/IP) + `/api/search` (30/min/IP). `docs/OPS.md` (G-4b).
- **Phase 9 (migration `20260611230000`)** : **P-FLEX** — RPC `set_reservation_item_price` (édition admin du prix FACTURÉ d'une ligne, **pending/confirmed UNIQUEMENT**, total recalculé atomiquement, audit **high-impact**) + édition inline dans `ReservationDrawer` (prop optionnelle — `/admin/ventas` ne la fournit pas) ; `Product.priceNote` = wording validé « Precio indicativo — se confirma en farmacia » ×3 ; `sitemap.ts` **revalidate 3600** (était devenu figé au build) ; `src/app/manifest.ts` ; `shipping.ts` doc LEGACY.
- **Phase L-2** : drawer **« Initialiser l'inventaire »** dans `/admin/stock` (quantité obligatoire + coût OPTIONNEL → réception si coût / ajuste sinon, stock résultant affiché, prix de vente + activation en option) + route dédiée **`PATCH /api/admin/products/[id]/active`** (`is_active` reste strippé de `productUpdate` — invariant anti-mass-assignment testé ; l'activation est une action explicite auditée = levier de la barrière L-3). `PLACEHOLDER_PRICE` centralisé.
- **Phase 8 — Next 16 MERGÉ dans main (2026-06-12, `baf5adf`)** : Next **16.2.9** épinglé (build **Turbopack** vert ~36 s, 63 routes prerendues, ISR/lang préservés) ; `next lint` → **ESLint CLI** (`eslint .`, flat config native eslint-config-next ; ⚠️ scanne TOUT le repo contrairement à `next lint` → ignores durcis `.next*/**`, scripts, tests — ne jamais déplacer un cache `.next-stale-*` DANS le repo) ; `revalidateTag(tag, { expire: 0 })` (2ᵉ arg obligatoire en 16) ; `@types/node` ^22 ; famille de règles « React Compiler » (react-hooks v7) désactivée = chantier séparé ; middleware.ts conservé (rename proxy.ts différé). Vérifié post-merge par 3 agents indépendants (config 18/18, runtime 19/19, e2e chromium complet) — y compris la première combinaison Next 16 + redesign catalogue « Rail editorial ». Reliquat cosmétique : devDep `@eslint/eslintrc` désormais inutilisée (candidate à l'uninstall).

**Reste à faire / propriétaire** : **L-1 reset** (353 cibles vérifiées — runbook étape 0, JAMAIS exécuté par l'agent, dump données avant — `docs/OPS.md` §1) ; poser `SENTRY_DSN` + uptime monitor `/api/health` + DNS Resend ; QA navigateur (email réservation après DNS, effacement de compte, édition prix, drawer init) ; décision merge `chore/next-16`. Différés documentés : passe axe complète, route `opengraph-image`, conversion `unstable_cache` → `use cache` (Next 16).

### Fait ✅ (session 2026-06-09 — promotions + merma + journal d'audit)

3 features livrées de bout en bout, plan validé par **10 agents indépendants** (`~/.claude/plans/bubbly-riding-eclipse.md`). **4 commits sur `main`** en paliers — `2feacee` (fondation : les **4 migrations** + schéma + types + schemas Zod + i18n + sidebar) → `7d5ad07` (code audit) → `13d1012` (code merma) → `814b9b2` (code promo). **4 migrations appliquées prod** (`20260609120000`→`20260609150000`). Vérif : tsc 0 · lint 0 · build vert · vitest 26/26 · parité i18n **2043×3** · advisors propres (seuls **2 WARN attendus** `effective_price` anon/authenticated, le GRANT EXECUTE anon est volontaire et n'expose aucun coût). `db/schema.sql` régénéré (2998 lignes, dump complet via `scripts/db-dump.sh`).

**1. Journal d'audit admin** (code `7d5ad07`, migration `20260609120000_audit_log.sql`) — table `audit_log` (`actor_id`, `action` create/update/delete, `entity`, `entity_id`, `summary` ES, `diff` jsonb, `is_high_impact` stocké ; RLS lecture **tous admins** via `is_user_admin`, écriture service-role). Helper **`src/lib/audit.ts` `recordAuditLog()`** : utilise **`after()` de `next/server`** (un `void (async)()` nu serait gelé après la réponse sur Vercel → logs perdus), no-op si `supabaseAdmin` null, ne throw jamais ; `sanitizeDiff` retire `file`/`imageFile` (blobs base64) + `password` et tronque les longues chaînes. **~25 routes admin / ~38 call-sites** instrumentés (1 appel par mutation, avant la réponse succès) ; **PII redigée** (téléphone client hors `diff` du quick-create ; diff réservation compact). Exclus : `banners/stats`, `set-locale`, tous les GET. Page **`/admin/logs`** + `GET /api/admin/logs` : filtres entité/action/auteur/dates + bascule « Solo alto impacto », résolution acteur via `profiles` + `auth.admin.listUsers` (early-exit borné), acteur NULL → « Sistema ». Sidebar `navLogs` (section Acceso).

**2. Merma / productos vencidos** (code `13d1012`, migration `20260609130000_stock_losses.sql`) — perte de stock → **dépense au coût (CMP)**. RPC **`record_stock_loss`** (SECURITY DEFINER, service-role, idempotente `client_token`) : verrou `FOR UPDATE`, lecture coût **côté serveur**, décrément `GREATEST(stock-qty,0)` (skip `stock IS NULL`), insère une charge catégorie **`merma`** dans `expenses` au `round(cost·qty,2)` + une ligne registre **`stock_losses`** (snapshot `unit_cost`, raison `vencido/danado/robo/ajuste`, lien `expense_id`). **Coût inconnu → décrément sans charge** (jamais 0). Garde-fou overflow (≤100M). Ne touche **jamais** `cost_price` ni `stock_entries` (registre 606). Catégorie `merma` ajoutée à la CHECK SQL + `CAT_LABEL` (« Mermas y pérdidas »), **hors** du `<select>` manuel d'`ExpensesPanel` (et hors `EXPENSE_CATEGORIES` Zod → POST manuel `merma` rejeté). UI : `StockLossDrawer` (action de ligne dans `/admin/stock`, preview `cost·qty`), i18n `Admin.stock.loss`. Le P&L agrège merma automatiquement (aucun double-comptage : les unités perdues ne deviennent jamais COGS).

**3. Campagnes promo** (code `814b9b2`, migrations `20260609140000_promotions.sql` + `20260609150000_reservations_effective_price.sql`) — remise **% ou montant fixe**, ciblant **produit / marque / gamme / tag**, datée. Tables `promotions` + `promotion_targets` (privées). **`effective_price(product_id, at)`** (SECURITY DEFINER, STABLE, `LEFT JOIN ranges`, **meilleur prix client = MIN** sur toutes les cibles) + vue publique **`v_product_pricing`** (`security_invoker = true` → respecte la RLS produits, **aucun** coût exposé). ⚠️ **Gotcha clé** (mémoire `promo-pricing-public-view-gotcha`) : exposer une valeur calculée par une fonction SECURITY DEFINER au public nécessite **`GRANT EXECUTE` à anon** (l'EXECUTE est vérifié contre l'invoker même dans une vue) **+** la vue en `security_invoker` (sinon elle bypasse la RLS et **fuit les produits inactifs**). Le prix effectif **alimente le snapshot des réservations** : `create_reservation` + `create_guest_reservation` réécrites (`pr.price` → `effective_price(pr.id, now())`, grants asymétriques préservés) → la **compta enregistre le CA remisé**. Affichage discount-aware (prix barré + badge `−X%`) sur **toutes** les surfaces via le helper **`src/lib/pricing.ts`** (`fetchEffectivePrices` batch + `applyPromo` : `oldPrice = max(base, old_price manuel)`) : catalogue, PDP (`ProductClient`+`PdpStickyBar`+JSON-LD), home (`HomeBestsellers`, barré net-new), `marques/[slug]`, `besoins/[slug]` (revalidate **300→60s**), favoris, panier (`/api/cart`+`CartLineItem`), recherche (défaut vente comptoir). RPC `set_promotion_targets` (swap atomique → pas de fenêtre d'orphelins). Admin **`/admin/promotions`** (CRUD + target picker) + `/api/admin/promotions` (+`[id]`, validation existence des cibles, audit). Sidebar `navPromotions`.

**Reste à faire** : **QA navigateur des 3 flux** (mutations réelles non testées : merma → stock+P&L ; promo → barré + réservation remisée en compta ; action admin → `/admin/logs`) ; `database.types.ts` a une divergence cosmétique mineure (8 hints `v_product_pricing` referencedRelation absents vs regen frais — inoffensif, se réconcilie au prochain `generate_typescript_types`). Pistes promo : tracking impressions/clics, programmation auto des statuts, `priority` comme vrai départage. Mémoires : `promo-merma-audit-features`, `promo-pricing-public-view-gotcha`.
