-- ============================================================================
-- get_catalogue_page : garde-fou v_tree contre une valeur scalaire dans p_pairs.
--
-- La v2 (20260611235000) calculait v_tree avec
--   WHERE jsonb_typeof(f.ranges) = 'array' AND jsonb_array_length(f.ranges) > 0
-- Postgres ne garantit PAS le court-circuit du AND : un appel anon direct avec
-- p_pairs = '{"Babe": "Corporal"}' (valeur scalaire) levait
-- `22023: cannot get array length of a scalar` → 400 sur un endpoint public.
-- Inatteignable via l'app (parseFilters/buildSelectionFromModel n'émettent que
-- des string[]), mais la fonction est GRANT EXECUTE TO anon — tout doit être
-- défensif. Les CTE internes (filtered, tag_base_pool) ne crashent pas (la
-- corrélation pair_brand = brand_name pré-filtre la ligne fautive) : seul
-- v_tree est corrigé, via un CASE qui neutralise les valeurs non-array.
--
-- CREATE OR REPLACE à signature INCHANGÉE → les grants existants sont
-- préservés, aucun DROP nécessaire. Seul le bloc DECLARE/v_tree change.
-- ============================================================================
