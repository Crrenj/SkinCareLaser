...
## État actuel (2026-05-21, post-P5)

**Branche `main`, synchronisée avec `origin/main`.** Dernier commit : `ebad106 feat(admin): /admin/users + /admin/newsletter + cleanup my-team`.
...
**5 sessions livrées entre `c37a915` et `ebad106`** :
...
1. **JSON-LD Product schema** sur `/product/[slug]` — rich snippets Google (Offer, AggregateRating si reviews un jour)
2. **3 `<img>` → `next/image`** : `CartDrawer`, `ProductClient` (×2) — impact LCP fiche produit
3. **Migration `banner_type_enum`** strict : la colonne reste `text` pour compat legacy. Quand toutes les lignes auront été re-sauvegardées via l'admin, créer l'enum strict.
