...
## Dernière session (2026-06-03 — refonte `/admin/annonce` + thème lisible)
...
- **Injection serveur** : `getThemeConfig()` (`src/lib/getThemeConfig.ts`) — lecture anon **sans cookies** + `unstable_cache` (tag `shop-theme-config`, `revalidateTag` au save admin) → **ne force pas le dynamic**, les pages `[locale]` restent SSG. Root layout pose `<html data-theme data-mode>` + script anti-flash. **Favicon par thème** via `ThemeFavicon` (client, lit `/api/theme` `no-store`) ; `src/app/favicon.ico` supprimé. **L'admin adopte le thème d'apparence** (`_AdminShell` pose `data-theme={siteTheme}` ; plus forcé terra) — seul le mode clair/sombre lui est propre. ⚠️ Les mappings `--color-*: var(--c-*)` doivent être redéclarés dans `[data-theme]` (sinon figés sur `:root` → `[data-theme]` imbriqué ne re-thématise pas ; mémoire `theme-nested-resolution-gotcha`).
