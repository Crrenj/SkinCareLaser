...
## État actuel (2026-05-22, post session 7 hygiène technique)

**Branche `main`.** Dernier commit pushé : `8d8ec14 feat(pdp): JSON-LD Product schema + next/image (CartEmpty + ConfirmationRecap)`.
**Working tree non commité** : session 7 (hygiène technique autonome, voir plus bas) — ~20 fichiers modifiés + 1 migration DB appliquée + 3 fichiers supprimés. À reviewer avant commit.
...
**6 sessions livrées entre `c37a915` et `8d8ec14` + 1 session non-commitée** :
...
### Session 7 (non-commitée, 2026-05-22) — Hygiène technique autonome

Exécutée en mode autonome après création du plan `~/.claude/plans/goofy-wobbling-teacup.md`. Pas de commit (règle Cursor `alwaysApply`).

- **Code mort retiré** (`git rm`) : `src/components/FiltersNew.tsx` (393 LOC), `src/components/admin/ImageUpload.tsx`, `src/components/admin/DirectImageUpload.tsx` — totalisaient ~700 LOC, 0 import dans `src/`. Les 3 `catch (error: any)` restants du projet disparaissent avec ces fichiers
- **`src/lib/constants.ts` + `src/lib/formatPrice.ts`** créés :
  - constants : `DEFAULT_CURRENCY`, `DEFAULT_LOCALE_TAG`, `LOW_STOCK_THRESHOLD`, `MAX_CART_QUANTITY`, `ADMIN_HOME_PATH`, `LOCALE_TAG_MAP`, helper `toLocaleTag(locale)`
  - formatPrice : `formatPrice(value, { locale?, fractionDigits? })` wrapper sur `Intl.NumberFormat`
  - 5 défauts `'DOP'` hardcodés migrés vers `DEFAULT_CURRENCY` (favoris, admin/reservations, api/products, api/search, useCart)
  - 13 appels `Intl.NumberFormat` migrés vers `formatPrice`, 7 déclarations `localeMap` dupliquées supprimées
  - Seul `account/reservations/page.tsx` conserve son `Intl.NumberFormat` direct (comportement défaut 0-3 chiffres, intentionnellement non centralisé pour ne pas changer le rendu)
- **`focus-visible` global** : sed sur les 64 occurrences de `focus:outline-none` dans 22 fichiers. Pattern : sur chaque ligne contenant `focus:outline-none`, tous les préfixes `focus:` sont renommés `focus-visible:` (préserve les `focus:ring-*`, `focus:border-*` adjacents en les transformant aussi). Les 13 cas isolés (sans `focus:ring-` ni `focus:border-` voisin) reçoivent `focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-clay-700`. Bloqueur WCAG 2.4.7 fermé. `grep "focus:outline-none" src/` → 0 résultat
- **Toaster sonner installé** : `npm install sonner` (sonner@2.0.7), `<Toaster richColors position="top-right" closeButton />` monté dans `src/app/admin/layout.tsx`. 27 `alert()` admin migrés :
  - 25 `alert('Erreur…')` → `toast.error(…)` (product, tags, stock, annonce, marques)
  - 2 `window.alert('… próximamente.')` → `toast.info(…)` (reservations)
  - Tous les fichiers admin reçoivent `import { toast } from 'sonner'`
- **Migration DB `20260522092810_set_search_path_security_definer.sql`** : appliquée via MCP. `ALTER FUNCTION ... SET search_path = public, pg_temp` sur les 9 fonctions sans config (`add_to_cart`, `cleanup_banner_positions`, `get_messages_stats`, `get_or_create_cart`, `is_user_admin`, `mark_message_as_read`, `remove_from_cart`, `reorder_banners`, `update_updated_at_column`). Advisor `function_search_path_mutable` désormais à 0
- **`npm uninstall framer-motion @supabase/auth-helpers-nextjs`** : refusé en permission pendant la session. Les 2 deps restent dans `package.json` (0 import dans `src/`). À retirer manuellement quand souhaité

Vérification finale :
- `npx tsc --noEmit` → 0 erreur
- `npm run test:unit -- --run` → 8/8
- `npm run lint` → 0 erreur, mêmes ~4 warnings préexistants (useWishlist + supabaseClient `_options`)
- `npm run test:smoke` → bloqué localement (`chrome-headless-shell` non installé : `npx playwright install` requis). CI le résoudra
- `mcp__supabase__get_advisors` (security) → `function_search_path_mutable` éliminé (les autres warnings sont des choix de design pré-existants : RLS service-role only, public buckets, SECURITY DEFINER intentionnellement exposés)

Avant commit : faire `git diff --stat` pour valider la portée, puis split éventuellement en 5 commits thématiques (dead code, constants/formatPrice, focus-visible, sonner, search_path migration).
...
4. ~~**`focus-visible` global**~~ ✅ session 7 (64 occ migrées)
...
7. **`confirm()` natifs** : il reste plusieurs `confirm()` dans l'admin (suppressions). À remplacer par une modale custom ou la primitive de `sonner` (`toast.message` avec actions) pour cohérence avec le toast system installé en session 7
...
17. ~~**Audit RPC `SECURITY DEFINER`** pour `SET search_path = public` manquants~~ ✅ session 7 (9 fonctions corrigées, advisor à 0 sur ce lint)
18. **`npm uninstall framer-motion @supabase/auth-helpers-nextjs`** (deps non utilisées dans `src/`, retrait refusé en session 7 — `npm uninstall` direct à exécuter manuellement)
